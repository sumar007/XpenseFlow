...
export const addTimeSheet = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to add time sheet", error: error.message });
  }
};
...
export const getTimeSheets = async (req, res) => {
  ...
  try {
    ...
    if (!employeeTimeSheets || employeeTimeSheets.length === 0) {
      return res
        .status(404)
        .json({ message: "No time sheets found for this employee" });
    }
    // Calculate the number of projects
    const numberOfProjects = employeeTimeSheets.reduce(
      (acc, curr) => acc + curr.projects.length,
      0
    );
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch time sheets", error: error.message });
  }
};
